...
const StoresSection: React.FC = () => {
  ...
  const stores: StoreInfo[] = [
    {
      id: 'billa',
      name: t('storesSection.storeLabels.billa'),
      logo: BillaLogo,
      products: 2500,
    ...
    {
      id: 'kaufland',
      name: t('storesSection.storeLabels.kaufland'),
      logo: KauflandLogo,
      products: 3200,
    ...
    {
      id: 'lidl',
      name: t('storesSection.storeLabels.lidl'),
      logo: LidlLogo,
      products: 2800,
    ...
    {
      id: 'tesco',
      name: t('storesSection.storeLabels.tesco'),
      logo: TescoLogo,
      products: 3600,
      accent: 'from-blue-700 to-red-600',
    },
  ];
  ...
  return (
    <section className="relative py-12 bg-brand-nude overflow-hidden">
      ...
      <div className="section-container relative z-10">
        <div className="max-w-3xl mb-8 reveal-animation" data-anim="left">
          <h2 className="text-3xl md:text-5xl font-bold text-brand-indigo">
            ...
            <span className="text-brand-primary">
              {t('storesSection.headingAccent')}
            </span>
          </h2>
          <p className="mt-4 text-brand-indigo/60 text-lg md:text-xl">
            {t('storesSection.subheading')}
          </p>
        </div>
          ...
          {stores.map((store) => (
            ...
            >
              ...
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-3 text-center px-4 opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-brand-primary/85 backdrop-blur-sm rounded-2xl">
                ...
                <div className="text-xs md:text-sm uppercase tracking-widest text-brand-secondary font-medium">
                  {t('storesSection.productsTracked')}
                </div>
              </div>
        ...
        {/* Key points */}
        <div
          className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8 reveal-animation"
          data-anim="up"
        >
          {[
            {
              icon: <RefreshCw className="w-5 h-5 text-brand-primary" />,
              text: t('storesSection.keyPoints.regularUpdates'),
            },
            {
              icon: <Bot className="w-5 h-5 text-brand-primary" />,
              text: t('storesSection.keyPoints.aiAutomation'),
            },
            {
              icon: <DatabaseZap className="w-5 h-5 text-brand-primary" />,
              text: t('storesSection.keyPoints.growingDatabase'),
            },
          ].map((point, i) => (
            <div
              key={i}
              className="flex items-center gap-3 bg-white/60 rounded-xl px-4 py-3 border border-brand-lilac/20"
            >
              <div className="flex-shrink-0 p-2 bg-brand-lilac/20 rounded-lg">
                {point.icon}
              </div>
